...
// 1. (BEST PRACTICE) Definisikan interface untuk bentuk data Anda
interface Question {
  type: string;
...
export default async function QuizCoverPage({ params }: { params: { summaryId: string } }) {
  ...
  // Lakukan type assertion agar TypeScript tahu ini adalah array dari Question
  const questions = quiz.questions as Question[];
  const questionCount = questions.length;

  // 2. Sekarang Anda tidak perlu `any` lagi, kode lebih aman dan jelas
